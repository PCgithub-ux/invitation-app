...
export default class InvitationResponse extends LightningElement {
    ...
    changeHandler(event){
        ...
        if (this.formData.Response === "Joyfully Accept") {
            this.isRsvpAccepted = true
        }else{
            this.isRsvpAccepted = false
        }
    }
    submitHandler(event){
        this.rsvpMessage=''
        ...
        submitResponse({
            ...
            if(result){
                ...
                console.log("checkandsetmessage called")
                this.checkAndSetMessage()
            }
        }).catch(error=>{
    ...
    checkAndSetMessage(){
        ...
        console.log("rsvpMessage", this.rsvpMessage)
    }
