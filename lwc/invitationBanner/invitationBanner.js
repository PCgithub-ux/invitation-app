...
export default class InvitationBanner extends LightningElement {
    ...
    // Paths to the static resource
    instagramImage = marriageInvitationAssets + '/marriageInvitationAssets/instagram.svg';
    twitterImage = marriageInvitationAssets + '/marriageInvitationAssets/twitter.svg';
    whatsappImage = marriageInvitationAssets + '/marriageInvitationAssets/whatsapp.svg';
    ...
    get bannerImage(){
        let themeName = marriageInvitationAssets + `/marriageInvitationAssets/${this.theme}.jpeg`;
     ...
     invitationDetailsHandler({data, error}){
         if(data){
             ...
             this.whatsappUrl=data.Whatsapp_Url__c
             this.twitterUrl=data.Twitter_Url__c
             this.instagramUrl=data.Instagram_Url__c
             if(!this.isEventDatePassed(data.Event_Date_and_Time__c)){
    ...
    isEventDatePassed(eventDate){
        const today = new Date()
        const eventdate = new Date(eventDate)
        if(today>eventdate){
            this.isEventDateNotPassed = false
            return true
        }else{
            this.isEventDateNotPassed = true
            return false
        }
    }
}